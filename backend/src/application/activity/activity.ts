...
export class ActivityService implements IAuditLogRepository {
  ...
  async logAction(auditLog: AuditLogEntity): Promise<void> {
    this.logger.info(
      `Logging action: ${auditLog.action} for entity: ${auditLog.entityType} with ID: ${auditLog.entityId} by user: ${auditLog.userId}`
    )
    await this.database.create(this.model, auditLog)
  }
  async getLogsByEntity(entityType: number, entityId: String): Promise<AuditLogEntity[]> {
  ...
  async getLogsByAction(action: number): Promise<AuditLogEntity[]> {
